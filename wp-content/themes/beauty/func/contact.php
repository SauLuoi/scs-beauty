...
ob_clean();
ob_start();
require_once(get_template_directory() . '/func/contact/admin.php');
// 問合せのフォーム内容はここで設定
function settingFormParam($slug = 'agent-recruitment')
{
    $formSettings = array();
    switch($slug)
    {
...
function originalContactConfirm($slug = 'agent-recruitment')
{
    ...
    foreach($validationParams as $key => $valid)
    {
        if(!isset($myContactErrors[$key]))
        {
            if(isset($valid['required']) && $valid['required'])
            {
                if(empty($values[$key]))
                {
                    if(is_array($values[$key]))
                    {
                        ...
                        if($key == 'agreement'){
                            $myContactErrors[$key] = 'Cần đồng ý với ' . $valid['name'];
            ...
            if(isset($valid['format']) && $valid['format'])
            {
                switch($valid['format'])
                {
                    ...
                    case 'tel':
                        ...
                        {
                            $myContactErrors[$key] = $valid['name'] . ' không đúng định dạng.';
                            //continue;
                        }
                        break;
                    case 'email':
            ...
            if(isset($valid['is_confirm']) && $valid['is_confirm'])
            {
                ...
                {
                    $myContactErrors[$key] = $valid['name'] . ' không đồng nhất với email xác nhận';
                    continue;
                }
            }
        }
...
function originalContactSend($slug = 'agent-recruitment')
{
    if( !session_id() ){session_start();}
    if($_SERVER["REQUEST_METHOD"] == "POST"){
