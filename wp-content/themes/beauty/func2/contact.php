...
ob_clean();
ob_start();
require_once(get_template_directory() . '/func2/contact/admin.php');
// 問合せのフォーム内容はここで設定
function settingFormParam1($slug = 'fda')
{
    $formSettings = array();
    switch($slug)
    {
...
// 画面フローはここで設定
function settingContactFlow1($slug)
{
    $flows = array();
    switch($slug)
    {
        case 'fda':
        default:
            $flows['confirm'] = '/jp/'.$slug.'/confirm/';
            $flows['thanks']  = '/jp/'.$slug.'/complete/';
            break;
    }
    return $flows;
}
...
function originalContactConfirm2($slug = 'fda')
{
    if( !session_id() ){session_start();}
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        if(!isset($_POST['_aiwave_token1']) || !wp_verify_nonce($_POST['_aiwave_token1'], 'aiwave_contact_form'))
        {
        ...
        $_SESSION[$slug] = array_merge($_POST,$_FILES);

        $validationParams = settingFormParam1($slug);
        $errors = originalContactValidation1($_POST, $validationParams);
        ...
        $flows = settingContactFlow1($slug);
...
function originalContactValidation1($values,$validationParams)
{
                ...
                if(empty($values[$key]))
                {
                    if(is_array($values[$key]))
                    {
                        ...
                        if($key == 'agreement'){
                            ...
                            $myContactErrors[$key] = $valid['name'] . 'は必須です。';
//                            $myContactErrors[$key] = 'Cần nhập ' . $valid['name'];
...
function originalContactSend1($slug = 'fda')
{
    if( !session_id() ){session_start();}
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if($toEmail)
        {
            ...
            $message = originalCreateContactMessage11($searches,$replaces,true, $slug);
            if(!mb_send_mail($toString, originalReplaceMailCommon1($subject,$searches, $replaces), $message, implode("\r\n",$myHeaders)))
            {
        ...
        $message = originalCreateContactMessage11($searches,$replaces, false,$slug);
        if(!mb_send_mail($ownerToString, originalReplaceMailCommon1($subject,$searches, $replaces), $message, implode("\r\n",$headers)))
        {
            $_SESSION[$slug]['common'] = 'メールの送信に失敗しました。お手数ですが時間をおいてやりなおしてください。';
            return $_SESSION[$slug];
        }

        stagegroup_consul_send1($message);
        unset($_SESSION[$slug]);
        $flows = settingContactFlow1($slug);
...
function originalCreateContactMessage11($searches, $replaces, $isUser = true,$slug)
{
    if($isUser)
    {
    ...
    return originalReplaceMailCommon1($body, $searches, $replaces);
}

function originalReplaceMailCommon1($body, $searches, $replaces)
{
...
function stagegroup_prefs_list1()
{
    return array(
        '1' => '北海道',
        '2' => '青森県',
...
function stagegroup_contact_trigger_list1()
{
    return array(
        '新規ホームページ制作を検討中',
        '既存ホームページのリニューアルを検討中',
...
function stagegroup_consul_send1($message,$convert_flg = false)
{
    $ch = curl_init();
    $post_url = 'https://consul.stagegroup.jp/common/form_db_api2.php';
    curl_setopt($ch, CURLOPT_URL, $post_url);
    curl_setopt($ch, CURLOPT_POST, 1);
    $post_data = stagegroup_consul_param_setup1($_POST,$message);
    if($convert_flg)
    {
...
function stagegroup_consul_param_setup1($val,$message)
{
    $form_query = array();
