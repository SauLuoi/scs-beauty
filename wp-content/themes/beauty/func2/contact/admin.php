<?php
add_action('admin_menu', 'contact_menu_page1');
function contact_menu_page1()
{
    add_menu_page('連絡先の設定2', '連絡先の設定2', 'manage_options', 'contact_menu_page1', 'add_contact_menu_page1', 'dashicons-email-alt', 59);
//	add_submenu_page( 'contact_menu_page', '予約フォーム設定', '予約フォーム設定', 'manage_options', 'contact_reservation_menu_page', 'add_contact_reservation_menu_page');
    add_action('admin_init', 'register_contact_setting2');
}

function register_contact_setting2()
{
    $addSlugs = array('fda');
...
function add_contact_menu_page1()
{
    create_common_contact_menu_page1('fda');
}


function create_common_contact_menu_page1($slug)
{
    $params = settingFormParam1($slug);
    $title = 'お問い合わせ設定';
    switch($slug)
    {
        case 'reservation':
